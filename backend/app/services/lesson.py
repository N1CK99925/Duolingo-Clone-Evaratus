...
def complete_lesson(db: Session, user_id: int, lesson_id: int) -> LessonCompleteResponse:
    ...
    # 1. Award XP (denormalized counters: users.total_xp + weekly leaderboard entry)
    xp_amount = lesson.xp_reward
    user.total_xp += xp_amount
    add_weekly_xp(db, user_id, xp_amount)

    # 2. Record per-lesson progress (one row per completed lesson).
    ...
    # Skill completion is derived: all lessons in the skill have progress rows.
